<div>
    ...
    <div class="border-2 border-silver-500 border-sketchy-lg hover:border-sketchy-md transition duration-700 ease-in-out p-4 mb-10 shadow-lg bg-white">
        @if($post->thumbnail !== null)
            <?php
            $finalCaption = ( !empty($caption) ) ? $caption : $post->attachment->first()->post_excerpt;
            ?>
            <a href="/blog/{{ $post->slug }}">
                <img alt="{{ $post->title }}" class="border-sketchy-lg hover:border-sketchy-md object-cover h-50 w-full" src="{{ $post->thumbnail->size(Corcel\Model\Meta\ThumbnailMeta::SIZE_LARGE)['url'] }}">
            </a>
            <p class="pt-1 text-gray-700 font-neucha italic text-center text-sm prose-light-blue">{!! $finalCaption !!}</p>
        @endif
